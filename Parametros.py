...
def matrizPraVetor(matriz):

        def matrizVetor(matriz):
            if isinstance(matriz, list): 
                return matriz[0]
            else:
                return matriz
        
        return [matrizVetor(elemento) for elemento in matriz]
...
def teste():
    ...
    for a in entradaUser:
        print('[',a,']','=',entradaUser[a])
    
    parametro = Parametros()
    parametro.receberParametros(entradaUser)

    m = parametro.getParametros()
...
def teste2():
    ...
    vetor = matrizPraVetor(entradaUser['material'])

    tape20 = tape.Tape("tapes_lista.txt",vetor)

    tape20.obterTape20()
    tape20.obterMat()
    #print(tape20.getTape20())

    parametro = Parametros()
    parametro.receberParametros(entradaUser)

    parametro.setMatb(tape20.getMat())
...
def teste3():
    ...
    parametro.setMatb(list(tape20.getMat().keys()))

    m = parametro.getParametros()

    admin = ParametrosAdmin('inputPraTestar.txt', m)
...
def teste4():
    ...
    parametro.setMatb(list(tape20.obterMats().keys()))

    m = parametro.getParametros()

    admin = ParametrosAdmin('inputPraTestarGAMIRN.txt', m)
